-- ascii_skull.lua — minimal skull-icon theme, error-forward.
--
-- Style 7 of the compact theme set. See nfo_format.lua's header comment for
...
return {
  format_tool_call = function(ctx)
    ...
    local tag = C.gray .. "☠" .. C.reset .. C.bold .. ctx.tool_name .. C.reset
    if summary == "" then
      return "\n" .. tag .. "\n"
    end
    return "\n" .. tag .. " " .. summary .. "\n"
  ...
  format_tool_result = function(ctx)
    ...
    return C.green .. "✓" .. C.reset .. " " .. C.dim .. first .. C.reset .. suffix
  end,
}
