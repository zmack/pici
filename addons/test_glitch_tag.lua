-- Tests for glitch_tag addon via pici --test harness.
local addon = dofile("addons/glitch_tag.lua")

pici.test.run("format_tool_call shows shaded-block prefix and tool name", function()
  local s = addon.format_tool_call({tool_name = "bash", call_id = "c1", args = {command = "echo hi"}})
  pici.test.ok(s:find("▓", 1, true) ~= nil and s:find("▒", 1, true) ~= nil and s:find("░", 1, true) ~= nil)
  pici.test.ok(s:find("bash", 1, true) ~= nil)
  pici.test.ok(s:find("echo hi", 1, true) ~= nil)
end)

pici.test.run("format_tool_result uses >> on success and !! on error", function()
...
pici.test.run("format_tool_result stays single line", function()
  ...
  pici.test.ok(s:find("(+2)", 1, true) ~= nil, tostring(s))
end)
...
pici.test.run("covers arbitrary tool names", function()
  local s = addon.format_tool_call({tool_name = "weird_tool", call_id = "c3", args = {}})
  pici.test.ok(s:find("weird_tool", 1, true) ~= nil)
end)
