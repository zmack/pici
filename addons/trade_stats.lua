-- trade_stats.lua — nfo "trader stats" footer theme.
--
-- Style 8 of the compact theme set. See nfo_format.lua's header comment for
-- the shared design constraints (SGR color only, single-line blocks).
--
-- Note: only `is_error` is available from the hook context (no duration or
-- real exit code is plumbed through), so EXIT is derived as 0/1 from
-- `is_error` rather than a fabricated process exit status.
...
return {
  format_tool_call = function(ctx)
    ...
    local tag = C.gray .. "[" .. C.reset .. C.bold .. ctx.tool_name .. C.reset .. C.gray .. "]" .. C.reset
    if summary == "" then
      return "\n" .. tag .. "\n"
    end
    return "\n" .. tag .. " " .. summary .. "\n"
  ...
  format_tool_result = function(ctx)
    ...
    local exit_code = ctx.is_error and 1 or 0
    local exit_color = ctx.is_error and C.red or C.green
    return first .. suffix .. "  " .. C.gray .. "│" .. C.reset .. " " .. exit_color .. C.bold .. "EXIT " .. exit_code .. C.reset
  end,
}
