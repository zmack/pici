-- Tests for block_meter addon via pici --test harness.
local addon = dofile("addons/block_meter.lua")

pici.test.run("format_tool_call shows a green loader meter and tool name", function()
  local s = addon.format_tool_call({tool_name = "bash", call_id = "c1", args = {command = "echo hi"}})
  pici.test.ok(s:find("▓▓▓", 1, true) ~= nil)
  pici.test.ok(s:find("\27[32m", 1, true) ~= nil, "call meter should be green")
  pici.test.ok(s:find("bash", 1, true) ~= nil)
  pici.test.ok(s:find("echo hi", 1, true) ~= nil)
end)

pici.test.run("format_tool_result meter turns red on error", function()
  ...
  pici.test.ok(ok_s:find("\27[32m", 1, true) ~= nil, "success meter should be green")
  pici.test.ok(err_s:find("\27[31m", 1, true) ~= nil, "error meter should be red")
end)
