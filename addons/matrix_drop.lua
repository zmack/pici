-- matrix_drop.lua — green cascading-glyph theme.
--
-- Style 3 of the compact theme set. See nfo_format.lua's header comment for
...
return {
  format_tool_call = function(ctx)
    ...
    local head = C.green .. "ﾊﾐﾋ" .. C.reset .. C.gray .. "▸" .. C.reset .. C.green .. C.bold .. ctx.tool_name .. C.reset
    if summary == "" then
      return "\n" .. head .. "\n"
    end
    return "\n" .. head .. " " .. summary .. "\n"
  ...
  format_tool_result = function(ctx)
    ...
    if ctx.is_error then
      return C.red .. "0x" .. C.reset .. C.gray .. "▸" .. C.reset .. " " .. C.red .. first .. C.reset
    end
    local suffix = extra > 0 and (" " .. C.gray .. "(+" .. extra .. ")" .. C.reset) or ""
    return C.green .. "01" .. C.reset .. C.gray .. "▸" .. C.reset .. " " .. C.dim .. first .. C.reset .. suffix
  end,
}
