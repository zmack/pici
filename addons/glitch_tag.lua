-- glitch_tag.lua — corrupted-block glitch theme.
--
-- Style 6 of the compact theme set. See nfo_format.lua's header comment for
...
return {
  format_tool_call = function(ctx)
    ...
    local tag = C.red .. "▓" .. C.yellow .. "▒" .. C.green .. "░" .. C.reset .. C.bold .. ctx.tool_name .. C.reset
    if summary == "" then
      return "\n" .. tag .. "\n"
    end
    return "\n" .. tag .. " " .. summary .. "\n"
  ...
  format_tool_result = function(ctx)
    ...
    if ctx.is_error then
      return C.red .. "!!" .. C.reset .. " " .. C.red .. first .. C.reset .. suffix
    end
    return C.gray .. ">>" .. C.reset .. " " .. C.dim .. first .. C.reset .. suffix
  end,
}
