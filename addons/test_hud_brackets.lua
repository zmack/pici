-- Tests for hud_brackets addon via pici --test harness.
local addon = dofile("addons/hud_brackets.lua")

pici.test.run("format_tool_call shows HUD-corner tag", function()
...
pici.test.run("format_tool_result shows OK/FAIL badges", function()
  ...
  pici.test.ok(err_s:find("⌊FAIL⌋", 1, true) ~= nil)
