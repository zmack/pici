...
// Phase 1 item 3.
TEST(MailboxCoordinator, LifecycleAndRouting) {
  const auto suffix =
  ...
  try {
    ...
    EXPECT_TRUE(first_child != children.end());
    ...
    try {
      static_cast<void>(coordinator.reply(
          child4, "root-only-session", MailboxPayload{.text = "wrong child"}));
    } catch (const MailboxError &error) {
    ...
    try {
      static_cast<void>(coordinator.reply(
          root_b, "exact-child-note", MailboxPayload{.text = "wrong root"}));
    } catch (const MailboxError &error) {
    ...
    EXPECT_TRUE(endpoint != child_agents.end());
    coordinator.store().send(EnqueueMailboxEntryRequest{
    ...
    EXPECT_TRUE(second_child != old_agents_after.end());
    EXPECT_TRUE(coordinator.store()
    ...
    EXPECT_TRUE(shutdown_endpoint != shutdown_agents.end());
