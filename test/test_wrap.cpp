...
// Keeps position assertions readable at call sites.
...
TEST(Wrap, WrapsAtSpacesNotMidWord) {
  ...
  for (const auto &row : rows)
    EXPECT_THAT(row, testing::SizeIs(testing::Le(8U)));
}

TEST(Wrap, OverlongTokenHardWraps) {
  {
    SCOPED_TRACE("single overlong token");

    const std::string token(40, 'x');
    const auto rows = wrapped_rows(token, 20, 0);
    EXPECT_EQ(rows.size(), 2U);
    for (const auto &row : rows)
      EXPECT_THAT(row, testing::SizeIs(testing::Le(20U)));
    std::string rebuilt;
    for (const auto &row : rows)
      rebuilt += row;
  ...
  {
    SCOPED_TRACE("overlong token resumes word wrapping");
    ...
    for (const auto &row : rows)
      EXPECT_THAT(row, testing::SizeIs(testing::Le(10U)));
  }
}

TEST(Wrap, AnsiEscapesAreZeroWidth) {
...
TEST(Wrap, PromptCarriedStartColumn) {
  ...
  if (rows.size() == 3) {
    ...
    // 6 columns.
    EXPECT_THAT(rows[0], testing::IsEmpty());
    EXPECT_EQ(rows[1], "hello");
    EXPECT_EQ(rows[2], "world");
  }
}

TEST(Wrap, CursorPositionAtWordWrapBoundary) {
  {
    SCOPED_TRACE("word-wrap boundary");
    ...
    // resume=6.
    const auto last_char_before_break = position_for("hello world", 8, 0, 4);
    EXPECT_EQ(last_char_before_break, pos(1, 4));
    ...
    EXPECT_EQ(on_dropped_space, pos(2, 0));

    // Offset 6 is 'w', the first character resumed on the new row.
    const auto first_char_after_break = position_for("hello world", 8, 0, 6);
    EXPECT_EQ(first_char_after_break, pos(2, 0));
  }
  {
    SCOPED_TRACE("hard-wrap fallback boundary");
    ...
    EXPECT_EQ(at_fallback_boundary, pos(2, 0));
    const auto last_of_first_row = position_for(token, 8, 0, 7);
    EXPECT_EQ(last_of_first_row, pos(1, 7));
  }
}

TEST(Wrap, FitsWithoutWrapping) {
  const auto breaks = plan_word_wrap("short", 80, 0);
  EXPECT_THAT(breaks, testing::IsEmpty());
}

TEST(Wrap, EndOfTextFullRowNormalizes) {
  // "12345678" (8 chars) exactly fills an 8-column row.
  const auto position = position_for("12345678", 8, 0, 8);
  EXPECT_EQ(position, pos(2, 0));
}
