...
TEST(MailboxStore, PersistenceClaimsAndCleanup) {
  const auto suffix =
  ...
  try {
    {
      ...
      EXPECT_TRUE(claimed.messages.front().claim_token.has_value());
    ...
    EXPECT_EQ(sqlite3_open(symlink_target.c_str(), &symlink_database),
              SQLITE_OK);
