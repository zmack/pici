...
int cmd_run(const cli::Args &args,
  ...
  auto configure_hooks = [&]() {
    ...
    auto mailbox_error = [](core::MailboxErrorCode code,
                            std::string_view message) {
      return nlohmann::json{
          {"error",
           {{"code", core::mailbox_error_code_to_string(code)},
            {"message", message}}}};
    };
    const std::weak_ptr<core::MailboxCoordinator> mailbox_weak = mailbox;
    auto mailbox_call = [mailbox_weak, mailbox_error](auto fn) {
      auto current = mailbox_weak.lock();
      if (!current)
        return mailbox_error(core::MailboxErrorCode::not_found,
      ...
      try {
        ...
        return mailbox_error(core::MailboxErrorCode::internal, error.what());
      }
    };
    auto agent_json = [](const core::AgentRecord &agent) {
    ...
    auto message_json = [](const core::MailboxMessage &message) {
      nlohmann::json result{
          ...
          {"kind", core::mailbox_message_kind_to_string(message.kind)},
    ...
    auto parse_target = [](const nlohmann::json &value) {
      if (!value.contains("target") || !value.at("target").is_object())
        throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                 "target must contain exactly one address");
      const auto &target = value.at("target");
      const bool has_session =
          target.contains("session_id") &&
          target.at("session_id").is_string() &&
          !target.at("session_id").get<std::string>().empty();
      const bool has_agent = target.contains("agent_id") &&
                             target.at("agent_id").is_string() &&
                             !target.at("agent_id").get<std::string>().empty();
      if (has_session == has_agent)
        throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                 "target must contain exactly one address");
      return core::MailboxTarget{
          .session_id =
              has_session ? std::optional<std::string>{target.at("session_id")}
                          : std::nullopt,
          .agent_id = has_agent
                          ? std::optional<std::string>{target.at("agent_id")}
                          : std::nullopt};
    };
    auto parse_kind = [](std::string_view value) {
      const auto kind = core::mailbox_message_kind_from_string(value);
      if (!kind)
        throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                 "unknown mailbox message kind");
      return *kind;
    };
    auto parse_kinds = [parse_kind](const nlohmann::json &value) {
      std::vector<core::MailboxMessageKind> kinds;
      if (!value.contains("kinds")) {
        kinds = {
            core::MailboxMessageKind::steer, core::MailboxMessageKind::note,
            core::MailboxMessageKind::request, core::MailboxMessageKind::reply};
        return kinds;
      }
    ...
    auto parse_text = [](const nlohmann::json &value) {
      if (!value.contains("text") || !value.at("text").is_string())
        throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                 "text must be a non-empty string");
      auto text = value.at("text").get<std::string>();
      if (text.empty() || text.size() > static_cast<std::size_t>(64) * 1024)
        throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                 "text must be between 1 and 65536 bytes");
      return text;
    };

    info.mailbox.self = [mailbox_call, agent_json](const nlohmann::json &,
                                                   const std::stop_token &) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        return agent_json(coordinator.self());
      });
    };
    info.mailbox.list = [mailbox_call, agent_json](const nlohmann::json &value,
                                                   const std::stop_token &) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        const bool include_self = value.value("include_self", false);
        const bool include_stale = value.value("include_stale", false);
        const auto limit = value.value("limit", std::size_t{100});
        if (limit == 0 || limit > 100)
          throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                   "limit must be between 1 and 100");
        const auto self = coordinator.self();
        auto agents = coordinator.list_agents(
            core::AgentQuery{.include_stale = include_stale,
    ...
    info.mailbox.send = [mailbox_call, parse_target, parse_text,
                         parse_kind](const nlohmann::json &value,
                                     const std::stop_token &) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        auto request =
            core::SendRequest{.target = parse_target(value),
                              .kind = parse_kind(value.value("kind", "note")),
                              .body = {.text = parse_text(value)}};
        if (request.kind != core::MailboxMessageKind::note &&
            request.kind != core::MailboxMessageKind::steer)
          throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                   "send kind must be note or steer");
        if (value.contains("reply_to") && !value.at("reply_to").is_null()) {
          if (!value.at("reply_to").is_string())
            throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                     "reply_to must be a string");
          request.reply_to_message_id = value.at("reply_to");
        }
        const auto receipt = coordinator.send(std::move(request));
        return nlohmann::json{
    ...
    info.mailbox.request = [mailbox_call, mailbox_error, parse_target,
                            parse_text,
                            message_json](const nlohmann::json &value,
                                          std::stop_token stop_token) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        auto request =
            core::SendRequest{.target = parse_target(value),
                              .kind = core::MailboxMessageKind::request,
                              .body = {.text = parse_text(value)}};
        const auto receipt = coordinator.send(std::move(request));
        const auto timeout = std::clamp<std::int64_t>(
            value.value("timeout_ms", std::int64_t{30'000}), 0, 60'000);
        const auto deadline = std::chrono::steady_clock::now() +
                              std::chrono::milliseconds(timeout);
        auto generation = coordinator.status().mailbox.latest_generation;
        for (;;) {
          if (stop_token.stop_requested())
            return mailbox_error(core::MailboxErrorCode::internal,
                                 "mailbox request cancelled");
          for (const auto &message : coordinator.inspect(core::InboxQuery{
                   .kinds = {core::MailboxMessageKind::reply}, .limit = 100})) {
            if (message.reply_to_message_id == receipt.message_id) {
              auto result = message_json(message);
              result["state"] = "replied";
              result["request_id"] = receipt.message_id;
              return result;
            }
          }
          const auto remaining =
          ...
          const auto change = coordinator.wait(
              core::WaitRequest{
                  ...
                  .timeout_ms = std::min<std::int64_t>(remaining.count(), 250),
                  .poll_interval_ms = 25},
              stop_token);
    ...
    info.mailbox.reply = [mailbox_call, parse_text,
                          message_json](const nlohmann::json &value,
                                        const std::stop_token &) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        if (!value.contains("message_id") ||
            !value.at("message_id").is_string())
          throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                   "message_id must be a string");
        const auto incoming = coordinator.inspect(core::InboxQuery{
            .message_id = value.at("message_id").get<std::string>(),
            .include_acknowledged = true,
            .limit = 1});
        if (incoming.empty())
          throw core::MailboxError(core::MailboxErrorCode::not_found,
                                   "mailbox message not found");
        const auto &original = incoming.front();
        auto receipt = coordinator.send(core::SendRequest{
            .target =
                core::MailboxTarget{.session_id = original.sender_session_id},
            .kind = core::MailboxMessageKind::reply,
            .body = {.text = parse_text(value)},
    ...
    info.mailbox.inbox = [mailbox_call, parse_kinds,
                          message_json](const nlohmann::json &value,
                                        const std::stop_token &) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        const auto limit = value.value("limit", std::size_t{50});
        if (limit == 0 || limit > 50)
          throw core::MailboxError(core::MailboxErrorCode::invalid_message,
                                   "limit must be between 1 and 50");
        const auto kinds = parse_kinds(value);
        const bool claim = value.value("claim", false);
        nlohmann::json result = nlohmann::json::array();
        if (claim) {
          for (const auto &message :
               coordinator
                   .claim(core::ClaimRequest{.kinds = kinds, .limit = limit})
                   .messages)
            result.push_back(message_json(message));
        } else {
          for (const auto &message : coordinator.inspect(
                   core::InboxQuery{.kinds = kinds, .limit = limit}))
            result.push_back(message_json(message));
        }
        return result;
      });
    };
    info.mailbox.ack = [mailbox_call](const nlohmann::json &value,
                                      const std::stop_token &) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        if (!value.contains("message_id") ||
            !value.at("message_id").is_string() ||
            !value.contains("claim_token") ||
            !value.at("claim_token").is_string())
          throw core::MailboxError(core::MailboxErrorCode::invalid_claim,
                                   "message_id and claim_token are required");
        coordinator.acknowledge(
            core::AcknowledgeRequest{.message_id = value.at("message_id"),
                                     .claim_token = value.at("claim_token")});
        return nlohmann::json{{"message_id", value.at("message_id")},
                              {"state", "acknowledged"}};
      });
    };
    info.mailbox.wait = [mailbox_call](const nlohmann::json &value,
                                       std::stop_token stop_token) {
      return mailbox_call([&](core::MailboxCoordinator &coordinator) {
        const auto timeout = std::clamp<std::int64_t>(
            value.value("timeout_ms", std::int64_t{30'000}), 0, 60'000);
        const auto result =
            coordinator.wait(core::WaitRequest{.after_generation = value.value(
                                                   "after_generation", 0LL),
                                               .timeout_ms = timeout,
    ...
    info.mailbox.status = [mailbox_call](const nlohmann::json &,
                                         const std::stop_token &) {
      return mailbox_call([](core::MailboxCoordinator &coordinator) {
