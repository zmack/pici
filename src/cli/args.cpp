...
bool parse_thinking(std::string_view s, ThinkingLevel &out) {
  if (s == "off")     { out = ThinkingLevel::off;     return true; }
  if (s == "minimal") { out = ThinkingLevel::minimal;  return true; }
  if (s == "low")     { out = ThinkingLevel::low;      return true; }
  if (s == "medium")  { out = ThinkingLevel::medium;   return true; }
  if (s == "high")    { out = ThinkingLevel::high;     return true; }
  if (s == "xhigh")   { out = ThinkingLevel::xhigh;    return true; }
  return false;
}
...
Args parse_args(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    auto need = [&](std::string_view flag) -> std::string_view {
      if (i + 1 >= argc) {
        result.diagnostics.push_back({true, std::string(flag) + " requires an argument"});
        return {};
      }
      return argv[++i];
    };
    ...
    if (arg == "--help" || arg == "-h") {
      ...
      if (!v.empty()) result.append_system_prompts.push_back(std::string(v));
    } else if (arg == "--thinking") {
      auto v = need("--thinking");
      if (!v.empty() && !parse_thinking(v, result.thinking)) {
        result.diagnostics.push_back({false,
          "invalid thinking level \"" + std::string(v) +
      ...
      for (char c : s) {
        if (c == ',') {
          if (!tok.empty()) { result.tools.push_back(tok); tok.clear(); }
        } else {
          tok += c;
        }
      }
      ...
      result.diagnostics.push_back({false, "unknown option: " + std::string(arg)});
    } else {
      result.messages.push_back(std::string(arg));
    }
  }
...
void print_help(const char *prog) {
  std::cout <<
    ...
    "  --append-system-prompt <t>  Append text to system prompt (repeatable)\n"
    ...
    "Examples:\n"
    "  " << prog << "\n"
    "  " << prog << " \"What files are in this directory?\"\n"
    "  " << prog << " -p \"Summarize main.cpp\"\n"
    "  " << prog << " --model openai/gpt-4o --thinking high \"Explain this code\"\n"
    "  " << prog << " --model deepseek-chat --thinking medium\n"
    "  " << prog << " --base-url http://localhost:8080/v1 --model mymodel\n"
    "  " << prog << " --list-models\n"
    "  " << prog << " --list-models gpt\n\n"
