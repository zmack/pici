...
static constexpr std::string_view USER_IMAGE_PLACEHOLDER =
    "(image omitted: model does not support images)";
static constexpr std::string_view TOOL_IMAGE_PLACEHOLDER =
...
std::vector<ContentBlock> replace_images(
    ...
    for (const auto& block : content) {
        if (std::holds_alternative<ImageContent>(block)) {
            if (!prev_was_placeholder) {
                result.push_back(TextContent{.text = std::string(placeholder)});
            }
...
std::vector<Message> transform_messages(
    ...
    auto downgrade = [&](const std::vector<ContentBlock>& content, std::string_view ph)
        -> std::vector<ContentBlock> {
        if (has_images) return content;
    ...
    for (const auto& msg : messages) {
        if (std::holds_alternative<UserMessage>(msg)) {
            ...
            if (has_images) {
                first_pass.push_back(um);
            } else {
                UserMessage copy = um;
                copy.content = downgrade(um.content, USER_IMAGE_PLACEHOLDER);
                first_pass.push_back(std::move(copy));
            }
            ...
            for (const auto& block : am.content) {
                if (const auto* tc = std::get_if<ThinkingContent>(&block)) {
                    if (tc->redacted) {
                        if (same_model) transformed_content.push_back(*tc);
                        continue;
                    }
                    if (same_model && tc->thinking_signature.has_value()) {
                        transformed_content.push_back(*tc);
                        continue;
                    }
                    if (tc->thinking.empty() || tc->thinking.find_first_not_of(" \t\n\r") == std::string::npos) {
                        continue;
                    }
                    if (same_model) {
                        transformed_content.push_back(*tc);
                    } else {
                        transformed_content.push_back(TextContent{.text = tc->thinking});
                    }
                } else if (const auto* text = std::get_if<TextContent>(&block)) {
                    if (same_model) {
                        transformed_content.push_back(*text);
                    } else {
                        transformed_content.push_back(TextContent{.text = text->text});
                    }
                    ...
                    transformed_content.push_back(std::move(tc_copy));
                } else {
    ...
    auto insert_synthetic = [&]() {
        ...
        for (const auto& tc : pending_tool_calls) {
            if (!existing_tool_result_ids.count(tc.id)) {
    ...
    for (const auto& msg : first_pass) {
        if (std::holds_alternative<AssistantMessage>(msg)) {
            insert_synthetic();
            const auto& am = std::get<AssistantMessage>(msg);
            if (am.stop_reason == StopReason::error || am.stop_reason == StopReason::aborted) {
                continue;
            }
            pending_tool_calls.clear();
            existing_tool_result_ids.clear();
