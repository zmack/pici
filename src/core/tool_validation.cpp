...
void apply_object_coercion(njson &obj, const njson &schema) {
  if (!obj.is_object())
    return;
...
void apply_array_coercion(njson &arr, const njson &schema) {
  if (!arr.is_array())
    return;
...
bool matches_json_type(const njson &value, const std::string &type) {
  if (type == "number")
...
njson coerce_with_schema(const njson &value, const njson &schema) {
  ...
  auto try_union = [&](const njson &schemas) -> njson {
    for (const auto &sub : schemas) {
      ...
      if (sub.contains("type")) {
        ...
        if (t.is_string() &&
            matches_json_type(candidate, t.get<std::string>())) {
          return candidate;
        }
      } else {
        return candidate;
  ...
  if (schema.contains("type")) {
    ...
    if (t.is_string()) {
      ...
      for (const auto &st : t) {
        if (st.is_string())
          schema_types.push_back(st.get<std::string>());
      }
    }
  }
...
json_validator &get_or_create_validator(const std::string &schema_str,
  ...
  auto &v =
      g_validator_cache.emplace(schema_str, json_validator{}).first->second;
...
std::string path_from_pointer(const nlohmann::json::json_pointer &ptr) {
...
public:
  ...
  void error(const nlohmann::json::json_pointer &ptr,
             const njson & /*instance*/, const std::string &message) override {
    errors.push_back({.path = path_from_pointer(ptr), .message = message});
  }
};
...
ToolValidator::validate(std::string_view tool_name,
  ...
  CollectingErrorHandler err_handler;
  {
    ...
    json_validator &validator =
        get_or_create_validator(std::string(schema_json_str), schema_json);
    try {
      validator.validate(arguments, err_handler);
    } catch (const std::exception &e) {
      err_handler.errors.push_back({.path = "root", .message = e.what()});
    }
  }

  if (err_handler.errors.empty())
    return std::nullopt;
...
validate_tool_arguments(const ToolDefinition &tool,
  ...
  return ToolValidator::validate(tool.name(), tool.schema().serialize(),
                                 mutable_arguments);
}
