...
make_mailbox_bindings(std::weak_ptr<MailboxCoordinator> coordinator) {
  ...
  result.self = [mailbox_call](const nlohmann::json &value,
    ...
    return mailbox_call(
        value, actor,
        [](MailboxCoordinator &coordinator, const AgentRuntimeIdentity &actor) {
    ...
    return mailbox_call(
        ...
        [&](MailboxCoordinator &coordinator,
          ...
          for (const auto &agent : agents) {
            ...
            entry["is_self"] = agent.agent_id == actor.agent_id;
    ...
    return mailbox_call(
        ...
        [&](MailboxCoordinator &coordinator,
          ...
          for (;;) {
            ...
            // durable and unacknowledged; callers can inspect or acknowledge
            // it.
            ...
            const auto change = coordinator.wait(
                WaitRequest{.after_generation = generation,
                            .timeout_ms =
                                std::min<std::int64_t>(remaining.count(), 250),
                            .poll_interval_ms = 25},
                stop_token);
  ...
  result.status =
      [mailbox_call](const nlohmann::json &value,
                     const std::optional<AgentRuntimeIdentity> &actor,
                     const std::stop_token &) {
        return mailbox_call(
            value, actor,
