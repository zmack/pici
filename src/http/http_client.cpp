...
int stop_token_progress(void* p, curl_off_t, curl_off_t, curl_off_t, curl_off_t) {
    return static_cast<const std::stop_token*>(p)->stop_requested() ? 1 : 0;
}

} // namespace

std::optional<HttpClient::Response> HttpClient::post(
    ...
    curl_easy_setopt(curl.handle, CURLOPT_TIMEOUT_MS, (long)timeout_ms.value_or(600000));
    ...
    curl_easy_setopt(curl.handle, CURLOPT_XFERINFODATA, const_cast<std::stop_token*>(&stop_tok));
...
bool HttpClient::post_streaming(
    ...
    auto write_callback = [](char* ptr, size_t size, size_t nmemb, void* data) -> size_t {
    ...
    WriteState state{std::string{}, std::move(on_line)};
